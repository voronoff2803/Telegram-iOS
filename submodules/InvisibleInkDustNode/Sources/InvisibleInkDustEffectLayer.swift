//
//  InvisibleInkDustEffectLayer.swift
//  InvisibleInkDustNode
//
//  Created by Alexey Voronov on 05.12.2023.
//
...
func metalLibrary(device: MTLDevice) -> MTLLibrary? {
    ...
    guard let path = mainBundle.path(forResource: "InvisibleInkDustMetalSourcesBundle", ofType: "bundle") else {
        return nil
    }
...
public final class InvisibleInkDustEffectLayer: MetalEngineSubjectLayer, MetalEngineSubject {
    ...
    public func updateContent(frame: CGRect, image: UIImage) {
    ...
    public func update(context: MetalEngineSubjectContext) {
        ...
        let particleColumnCount = Int(bounds.width)
        let particleRowCount = Int(bounds.height)
        let particleCount = particleColumnCount * particleRowCount
        
        if self.particleBuffer == nil {
        ...
        let _ = context.compute(state: DustComputeState.self, commands: { [weak self] commandBuffer, state in
            ...
            guard let particleBuffer = self.particleBuffer else {
                return
            }
            ...
            if !self.particleBufferIsInitialized {
                self.particleBufferIsInitialized = true
